    ...
    for div in divs:
        ...
        if h3 and h3.get_text().find(category) != -1:
            ...
            if not subcategories:
                for p in ps:
                    a = p.find('b', recursive=False).find('a')
                    if a:
                        subtitle = a.get_text()
                ...
                sum = len(subcategories)
                for p in ps:
                    a = p.find('b', recursive=False).find('a')
                    if a and a.get_text() in subcategories:
                        subtitle = a.get_text()
                        print('%s processing %d/%d' % (subtitle, count, sum))
                        get_item(subtitle)
                        count += 1
...
def get_html(item_name):
    ...
    proxy = random.choice(proxies)
    try:
        item_response = requests.get(item_url, timeout=5, proxies=proxy, headers=headers)
    except Exception as e:
...
if __name__ == '__main__':
    print('====head====')
    dir_name = '管理百科'
    get('管理百科', '管理咨询', '质量管理', '战略管理', '流程管理', '组织管理', '物资管理', '知识管理', '企业文化', '公共关系', '目标管理', '成本管理', '知识产权', '创新', '危机管理', '行政管理', '招标投', '供应商管理')
    print('====tail====')
    print('====head====')
    dir_name = '法律百科'
    get('法律百科')
